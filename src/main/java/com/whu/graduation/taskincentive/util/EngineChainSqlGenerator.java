...
public class EngineChainSqlGenerator {
    ...
    private void writeK6Template(String outputFile, String bearerToken) throws IOException {
        ...
        try (PrintWriter out = new PrintWriter(new FileWriter(outputFile))) {
            ...
            out.println("const TARGET_MODE = __ENV.TARGET_MODE || 'async'; // async|sync|noop");
            ...
            out.println("const NOOP_ENDPOINT = '/api/benchmark/noop';");
            ...
            out.println("  }");
            out.println("  if (TARGET_MODE === 'noop') {");
            out.println("    return 'noop';");
            out.println("  }");
            ...
            out.println("  const endpoint = endpointType === 'sync'");
            out.println("    ? SYNC_ENDPOINT");
            out.println("    : endpointType === 'noop'");
            out.println("      ? NOOP_ENDPOINT");
            out.println("      : ASYNC_ENDPOINT;");
            ...
            out.println("  }");
            out.println();
            out.println("  const reqOptions = { headers, tags: { endpoint_type: endpointType, scenario_name: exec.scenario.name || 'na' } };");
            out.println("  const res = endpointType === 'noop'");
            out.println("    ? http.get(`${BASE_URL}${endpoint}`, reqOptions)");
            out.println("    : http.post(`${BASE_URL}${endpoint}`, JSON.stringify(payload), reqOptions);");
